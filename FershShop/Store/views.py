...
def login(request):
    ...
    if request.method == 'POST':
        ...
        password = request.POST.get('password')
        if username and password:
            user_sql = Seller.objects.filter(username=username).first()
            if user_sql:
                # 对密码进行加密
                ...
                if user_sql.password == web_password and cookies == 'login_page':
                    ...
                    # 增加校验 校验是否有店铺
                    store = Store.objects.filter(user_id=user_sql.id).first()
...
def cookie_user(fun):
    def inner(request, *args, **kwargs):
        ...
        if c_username and s_username and c_username == s_username:
            ...
            user_sql = Seller.objects.filter(username=c_username).first()
            if user_sql:
                # 返回页面
...
def add_goods(request):
    ...
    if request.method == 'POST':
        ...
        goods.good_type = GoodsType.objects.get(id=good_type)
...
def details_goods(request, good_id):
    # 获取商品信息
    goods_data = Goods.objects.filter(id=good_id).first()
...
#     if keywords:
#         # 模糊查询 good_name 是要查询数据库的关键词 ，__contains  是模糊查询
#         list_goods_type = Goods.good_type(good_name__contains=keywords)
#     else:
#           #最终效果 三表查询，可以查询出一条数据，具体优化还需要原生数据库语句
#         for i in good:
#             list_goods_type = i.good_type_id
#             # print(list_goods_type)# 1 2 3 1 可以查到食品对应的类型ID
#             good_type = GoodsType.objects.filter(id =list_goods_type )
#             # print(good_type)# 查询
#     # 对list_goods的数据进行分页，每3条数据为一页
...
def list_goods_type(request):
    ...
    if keywords:
        # 模糊查询 good_name 是要查询数据库的关键词 ，__contains  是模糊查询
        lists_goods_type = GoodsType.objects.filter(good_name__contains=keywords)
    else:
        lists_goods_type = GoodsType.objects.all()
        # 对list_goods的数据进行分页，每3条数据为一页
...
def del_goods_type(request):
    ...
    GoodsType.objects.get(id=da).delete()
    return HttpResponseRedirect('/Store/list_goods_type')


def order_list(request):
    # 查询店铺中未处理的订单  因为前面在认证的时候  has_store为店铺id
    store_id = request.COOKIES.get('has_store')
    orderList = OrderDetail.objects.filter(order_id__order_status=2, good_store=store_id)

    return render(request, 'store/order_list.html', locals())


def order_list_all(request):
    # 当点击处理订单以后，将状态修改成3，并显示在所有列表中
    if request.method == 'POST':
        operation = request.POST.get('name')
        # 查询店铺id
        store_id = request.COOKIES.get('has_store')
        # 查询商品列表
    ...
    return render(request, 'store/order_list_all.html', locals())
...
class UserViewSet(viewsets.ModelViewSet):
    ...
    filterset_fields = ['good_name', 'good_price']  # 进行查询的字段
...
def ajax_list_goods(request):
    return render(request, 'store/ajax_list_goods.html')


# 定义发送邮件函数 里面只有一行代码，其余内容全部被封装到send_mail中
